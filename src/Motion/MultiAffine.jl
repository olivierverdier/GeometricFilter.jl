...
"""
MultiAffineMotion(G, M, conv) = MultiAffineMotion{typeof(conv),typeof(_get_group_operation_action(G, conv)),typeof(G),typeof(M)}(G, M)

get_action(m::MultiAffineMotion{TAD}) where {TAD} = _get_group_operation_action(m.G, TAD())
...
get_dynamics(m::MultiAffineMotion{LeftAction}, χ)  = _lin(m, χ)
function get_dynamics(m::MultiAffineMotion{RightAction}, χ) 
