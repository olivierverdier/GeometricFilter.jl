...
struct TranslationMotion{TG,TV,TAD<:ActionDirection} <: AbstractAffineMotion
    G::TG
    vel::TV # in Alg(G)
end
...
"""
TranslationMotion(G,vel,dir=LeftAction()) = TranslationMotion{typeof(G), typeof(vel), typeof(dir)}(G,vel)
...
get_dynamics(m::TranslationMotion{TG,TV,LeftAction}, u) where {TG,TV} =  -adjoint_action(m.G, u, m.vel)
# TODO: should be inverse adjoint action here?
get_dynamics(m::TranslationMotion{TG,TV,RightAction}, u) where {TG,TV} =  -adjoint_action(m.G, inv(m.G, u), m.vel)
